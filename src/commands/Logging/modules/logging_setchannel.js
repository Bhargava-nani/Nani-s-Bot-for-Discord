        ...
        try {
            if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                return InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        errorEmbed(
                            'Permission Denied',
                            'You need **Administrator** permissions to change log channels.'
                        ),
                    ],
                });
            }
            ...
            if (logChannel) {
                ...
                if (
                    ...
                    return InteractionHelper.safeEditReply(interaction, {
                        embeds: [
                            errorEmbed(
                                '⚠️ Bot Permission Error',
                                `I need **Send Messages** and **Embed Links** in ${logChannel}.`
                            ),
                        ],
                    });
                }
            ...
            return InteractionHelper.safeEditReply(interaction, {
                embeds: [
                    errorEmbed(
                        '❓ No Option Provided',
                        'Use either `channel` or `disable: true`.'
                    ),
                ],
            });
        } catch (error) {
